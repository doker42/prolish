...
class PaypalController extends Controller
{
    ...
    public function expressCheckout(Request $request) {
        ...
        $recurring = $request->get('recurring', false) ? true : false;

        // get new invoice id
        $invoice_id = Invoice::count() + 1;

        // Get the cart data
        $cart = $this->getCart($recurring, $invoice_id, $request->get('id'));

        // create new invoice
        $invoice = new Invoice();
        $invoice->title = $cart['invoice_description'];
        $invoice->price = $cart['total'];
        $invoice->membership_id = $request->get('id');
    ...
    public function expressCheckoutSuccess(Request $request) {
        ...
        // get cart data
        $cart = $this->getCart($recurring, $invoice_id, $invoice->membership_id);
    ...
    private function getCart($recurring, $invoice_id, $membership_id)
    {
        ...
        if ($recurring) {
            return [
                ...
                'return_url' => url('/paypal/express-checkout-success?recurring=1&id=' . $membership->id),
                ...
                'total' => $membership->price, // Total price of the cart
            ];
        }
        ...
        return [
            ...
            'items' => [
                [
                    'name' => 'Storage update (' . $membership->title . ')',
                    'price' => $membership->price,
                    'qty' => 1
                ]
            ],
            ...
            // return url is the url where PayPal returns after user confirmed the payment
            'return_url' => url('/paypal/express-checkout-success?id=' . $membership->id),
            // every invoice id must be unique, else you'll get an error from paypal
            'invoice_id' => config('paypal.invoice_prefix') . '_' . $invoice_id,
            'invoice_description' => "Order #" . $invoice_id . " Invoice",
            'cancel_url' => url('/'),
